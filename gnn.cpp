...
void BpNet::training( std::vector<sample> sampleGroup, double threshold)
{
...
void BpNet::predict(std::vector<sample>& testGroup)
{
...
void BpNet::setInput( std::vector<double> sampleIn)
{
    for (int i = 0; i < innode; i++) inputLayer[i]->value = sampleIn[i];
}

void BpNet::setOutput( std::vector<double> sampleOut)
{
