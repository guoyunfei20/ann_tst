...
int main()
{
    ...
    // 学习样本
    std::vector<double> samplein[4];
    std::vector<double> sampleout[4];
    ...
    std::vector<sample> sampleGroup(sampleInOut, sampleInOut + 4);
    testNet.training(sampleGroup, 0.0001);

    // 测试数据
    std::vector<double> testin[4];
    std::vector<double> testout[4];
    ...
    std::vector<sample> testGroup(testInOut, testInOut + 4);
